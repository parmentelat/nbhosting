...
* `/etc/sysconfig/docker` is the place where we `/nbhosting/dockers` as being `docker`'s workspace
...
* create a course (the git repo in fact) 
  ...
  will go in `/nbhosting/courses-git/flotbioinfo/`
...
* as it will create the actual course notebooks master area 
  ...
  in `/nbhosting/courses/flotbioinfo`
...
* also each docker container can be probed for its logs
  * `docker logs flotbioinfo-x-thestudentid`
